...
export function markOpening() {
  try { sessionStorage.setItem(OPENING_KEY, "1"); } catch { /* ignore */ }
}
...
export function BrowserWindow({ children }: { children: ReactNode }) {
  ...
  return (
    <div className={cn("relative z-10 flex min-h-dvh flex-col", !maximized && "md:pb-8 md:pl-[112px] md:pr-6 md:pt-5 lg:pr-10")}>
      <div className={cn("os-window flex min-h-dvh flex-1 flex-col md:min-h-[calc(100dvh-52px)]", opening && "window-open", minimizing && "window-minimize", maximized && "rounded-none")}>
        <div className="sticky top-0 z-40 bg-[var(--os-face)]">
